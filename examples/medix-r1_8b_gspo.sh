...
MODEL_PATH="${MODEL_PATH:-./checkpoints/medix-sft-8b-merged}"
N_GPUS="${N_GPUS:-2}"

if [[ ! -f "$MODEL_PATH/config.json" ]] || ! compgen -G "$MODEL_PATH/*.safetensors" > /dev/null; then
    echo "SFT merged model not found: $MODEL_PATH"
    echo "Run the LLaMA-Factory SFT and merge stages first:"
    echo "  llamafactory-cli train examples/qwen3vl_8b_lora_sft.yaml"
    echo "  llamafactory-cli export examples/qwen3vl_8b_lora_merge.yaml"
    exit 1
fi
